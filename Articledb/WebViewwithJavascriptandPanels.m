...
- (void)searchFor:(NSString *)string
        direction:(BOOL)directionFlag
    caseSensitive:(BOOL)caseSensitiveFlag
             wrap:(BOOL)wrapFlag {

  [self
      evaluateJavaScript:@"typeof SearchResults"
       completionHandler:^(NSString *result, NSError *error) {
         ...
         NSString *startSearch =
             [NSString stringWithFormat:
                           ...
                           string, directionFlag ? @"true" : @"false",
                           caseSensitiveFlag ? @"false" : @"true",
                           wrapFlag ? @"true" : @"false"];

         [self.findpanelwindowcontroller.message
             setStringValue:@"Searching ..."];
         ...
         [self evaluateJavaScript:startSearch
                completionHandler:^(NSString *result, NSError *error) {
                  if (error == nil) {
                    ...
                    [self
                        evaluateJavaScript:@"SearchResults.length"
                         completionHandler:^(NSString *result, NSError *error) {
                           if (error == nil && result != nil) {

                             [self.findpanelwindowcontroller.message
                                 setStringValue:
                                     [NSString stringWithFormat:@"%@ results",
                                                                result]];
                           }
                         }];
                  }
...
- (void)webView:(WKWebView *)webView
    runOpenPanelWithParameters:(WKOpenPanelParameters *)parameters
             ...
             completionHandler:(nonnull void (^)(NSArray<NSURL *> *_Nullable))
                                   completionHandler {
...
- (void)webView:(WKWebView *)webView
  ...
  if ([navigationAction.request.URL.absoluteString containsString:@"blob"]) {
    ...
    NSDictionary *hdr = navigationAction.request.allHTTPHeaderFields;
    if ([hdr objectForKey:@"Referer"]) {
      NSURL *ref = [NSURL URLWithString:hdr[@"Referer"]];
      NSURLComponents *com = [NSURLComponents componentsWithURL:ref
                                        resolvingAgainstBaseURL:NO];
      NSMutableDictionary *par = [[NSMutableDictionary alloc] init];
      for (NSURLQueryItem *item in com.queryItems) {
        par[item.name] = item.value;
      }
      if ([par objectForKey:@"saveName"]) {
        self.saveName = [par[@"saveName"] lastPathComponent];
      }
    }
...
- (void)userContentController:(WKUserContentController *)userContentController
  ...
  if ([message.name isEqualToString:@"Blob"]) {
    ...
    savePanel.nameFieldStringValue = self.saveName;
                   ...
                   NSData *body = [[NSData alloc]
                       initWithBase64EncodedString:
                           [NSString
                               stringWithString:
                                   [message.body
                                       substringFromIndex:range.location + 7]]
                                           options:
